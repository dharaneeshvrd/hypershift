...
var (
	unavailableCOSInstance = func(name string) error { return fmt.Errorf("%s COS instance unavailable", name) }

	// IBM Cloud catalog id for Cloud Object Storage(COS) service, can use this to list the COS instances in user account
	cosResourcePlaneId = "744bfc56-d12c-4866-88d5-dac9139e0e5d"

	// cosEndpoint returns COS endpoint url for the given location
	cosEndpoint = func(location string) string {
		return fmt.Sprintf("s3.%s.cloud-object-storage.appdomain.cloud", location)
	}
)
...
func createCOSClient(serviceInstanceCRN string, location string) (*s3.S3, error) {
	...
	serviceEndpoint := getCustomEndpointUrl(cosService, cosEndpoint(location))
...
func findCOSInstance(rcv2 *resourcecontrollerv2.ResourceControllerV2, cosInstanceName string, resourceGroupID string) (*resourcecontrollerv2.ResourceInstance, error) {
	instances, resp, err := rcv2.ListResourceInstances(
		&resourcecontrollerv2.ListResourceInstancesOptions{
			...
			ResourcePlanID:  &cosResourcePlaneId,
		},
	)
	if err != nil {
